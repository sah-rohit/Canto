...
export async function checkRateLimit(): Promise<RateLimitStatus> {
  ...
  try {
    ...
    if (serverRes.ok) {
      ...
      const remaining = Math.max(0, Number((DAILY_LIMIT - localCount).toFixed(1)));
  ...
  const remaining = Math.max(0, Number((DAILY_LIMIT - state.count).toFixed(1)));
...
export async function recordSearch(cost: number = 1): Promise<void> {
  ...
  if (!state || state.date !== today) {
    ...
    const newCount = Number((state.count + cost).toFixed(1));
    saveState({ ...state, count: newCount });
  }
}
