...
import { isOPFSAvailable, opfsWrite } from './opfs';
import { isFSAccessAvailable, fsWrite, getStoredFolderHandle } from './fsAccess';
...
export async function dbSaveCache(
  ...
  try {
    ...
    // Exact Preservation Protocol
    const slug = slugify(topic);
    const timestamp = Date.now();
    const enc = new TextEncoder();

    // 1. Display Record
    const displayContent = `# ${topic}\n\n${content}\n\n\`\`\`\n${typeof asciiArt === 'string' ? asciiArt : asciiArt?.art || ''}\n\`\`\`\n`;
    const displayBlob = enc.encode(displayContent);

    // 2. Context Record
    const contextRecord = {
      systemPrompt: "You are an encyclopedic AI with rich ASCII art capability...",
      conversationHistory: [{ role: "user", content: topic }, { role: "assistant", content }],
      userQuery: topic,
      modelParameters: { temperature: 0.7, topP: 0.95 },
      rawResponse: content,
      tokenUsage: {
        promptTokens: Math.round(wordCount * 0.4),
        responseTokens: Math.round(wordCount * 1.3),
        totalTokens: Math.round(wordCount * 1.7)
      },
      finishReason: "stop",
      timestamp
    };
    const contextBlob = enc.encode(JSON.stringify(contextRecord, null, 2));

    // Save to OPFS fallback
    if (isOPFSAvailable()) {
      await opfsWrite(`encyclopedia/entries/${slug}.md`, displayBlob).catch(() => {});
      await opfsWrite(`encyclopedia/contexts/ctx-${slug}-${timestamp}.json`, contextBlob).catch(() => {});
    }

    // Save to Layer 3 File System Access Drive Mode
    if (isFSAccessAvailable()) {
      const handle = await getStoredFolderHandle();
      if (handle) {
        await fsWrite(`encyclopedia/entries/${slug}.md`, displayBlob, handle).catch(() => {});
        await fsWrite(`encyclopedia/contexts/ctx-${slug}-${timestamp}.json`, contextBlob, handle).catch(() => {});
      }
    }
  } catch (e) {
...
export async function dbGetStorageLimit(): Promise<number> {
  return dbGetSetting('browser_limit_mb', 2048);
}

export async function dbSetStorageLimit(limitMB: number): Promise<void> {
  return dbSetSetting('browser_limit_mb', limitMB);
}
