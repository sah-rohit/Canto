...
export function initWriteQueue(deviceId: string): void {
  ...
  _retryInterval = setInterval(() => {
    if (_pendingStores.size > 0) {
      flushToDisk().catch(e => console.warn('[CantoStore] Background retry flush error:', e));
    }
  }, 30000);
}

export function getSyncStatus(): "synced" | "syncing" | "local" | "error" {
  if (_isFlushing) return "syncing";
  if (_pendingStores.size > 0) return "local";
  return "synced";
}
