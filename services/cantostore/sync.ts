...
export async function initiateSync(targetPeerId?: string): Promise<{ peerId: string; offer: string }> {
  const identity = await getOrCreateDeviceIdentity();
  const conn = new RTCPeerConnection(RTC_CONFIG);
  const channel = conn.createDataChannel('cantostore-sync', { ordered: true });

  const usePeerId = targetPeerId || `temp_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
  const session: RTCSession = { peerId: usePeerId, conn, channel, state: 'connecting' };
  _sessions.set(usePeerId, session);

  setupDataChannel(channel, usePeerId, identity);
  ...
  return { peerId: usePeerId, offer: btoa(offerBundle) };
}
...
export async function completeSync(answerBase64: string): Promise<void> {
  const identity = await getOrCreateDeviceIdentity();
  const answerBundle = JSON.parse(atob(answerBase64));
  const peerId = answerBundle.deviceId;

  let session: RTCSession | undefined;
  for (const [sId, s] of _sessions) {
    if (sId.startsWith('temp_') || sId === peerId) {
      session = s;
...
async function handleSyncMessage(
  ...
  if (msg.type === 'update') {
    ...
    logSync({
      ...
      peerId,
      direction: 'pull',
      storesAffected: ['history', 'cache', 'favorites', 'codex', 'notes', 'graphs', 'artHistory', 'collections', 'analytics', 'folders', 'settings'],
      recordCount: 0,
      success: true,
    });
  }
...
export async function generateQRPayload(): Promise<{ code: string; payload: string }> {
  ...
  const { offer } = await initiateSync();
  return { code, payload: offer };
}
