...
export async function detectExternalChanges(deviceId: string): Promise<string[]> {
  const mismatched: string[] = [];
  try {
    const handle = await getStoredFolderHandle();
    if (!handle) return mismatched;

    const manifest = await readManifest();
    if (!manifest) return mismatched;

    for (const [path, entry] of Object.entries(manifest.entries)) {
      const data = await fsRead(path, handle);
      if (data) {
        const hash = await sha256Hex(data);
        if (hash !== entry.hash) {
          mismatched.push(path);
          // Auto-sync back to IndexedDB mirror
          const repaired = await repairFile(entry, deviceId);
          if (repaired) console.log(`[CantoStore] Auto-synced external edit for: ${path}`);
        }
      }
    }
  } catch {}
