...
<body>
  ...
  const renderClickableText = (text: string) => {
    ...
    if (inArticleSearch.trim()) {
      const q = inArticleSearch.trim();
      const regex = new RegExp(`(${q.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&')})`, 'gi');
      parts = text.split(regex);
      return parts.map((chunk, idx) => {
        if (chunk.toLowerCase() === q.toLowerCase()) {
          return (
            <mark
              key={`search-match-${idx}`}
              style={{ backgroundColor: 'var(--accent-color)', color: 'var(--bg-color)', padding: '0.1rem 0.2rem' }}
            >
              {chunk}
            </mark>
          );
        }
        return renderClickableOriginal(chunk);
      });
    }
    return renderClickableOriginal(text);
  };
